...
contract VaultPay is ReentrancyGuard {
    ...
    function claimPayment(uint256 paymentId) external nonReentrant {
        ...
        if (uint64(block.timestamp) > payment.deadline) revert PaymentExpired();
        ...
        emit PaymentClaimed(paymentId, recipient, amount);
    }
    ...
    function cancelPayment(uint256 paymentId) external nonReentrant {
        ...
        if (uint64(block.timestamp) <= payment.deadline) revert PaymentNotExpired();
        ...
        emit PaymentCancelled(paymentId, payer, amount);
    }
